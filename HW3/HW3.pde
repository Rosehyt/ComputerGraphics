...
void cameraControl(){
    ...
    Vector3 forward = Vector3.sub(lookat, cam_position);
    Vector3 right   = Vector3.cross(forward, up);

    if (keyPressed) {
        switch(key){
            case 'w': cam_position = Vector3.add(cam_position, forward.mult(speed)); break;
            case 's': cam_position = Vector3.sub(cam_position, forward.mult(speed)); break;
            case 'a': cam_position = Vector3.sub(cam_position, right.mult(speed)); break;
            case 'd': cam_position = Vector3.add(cam_position, right.mult(speed)); break;
            case 'q': cam_position.y += speed; break;
            case 'e': cam_position.y -= speed; break;
        }
    }
